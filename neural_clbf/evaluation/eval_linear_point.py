...
def plot_linear_satellite(args):
    # Load the checkpoint file. This should include the experiment suite used during
    # training.
    log_file = args.log
    # log_file = "logs/linear_satellite_cbf/tanh/commit_7390ab2/version_0/checkpoints/epoch=20-step=3695.ckpt"
    neural_controller = NeuralCBFController.load_from_checkpoint(log_file, strict=False)
    print(neural_controller.disable_gurobi)
    ...
    neural_controller.experiment_suite.experiments[0].domain = [(-2.5, 2.5), (-2.5, 2.5)]
    ...
    # Plot the trajectory
    ax.plot(traj_df["$x$"], traj_df["$y$"], traj_df["$z$"], "black")
    ax.set_xlim([-2.0, 2.0])
    ax.set_ylim([-2.0, 2.0])

    ax.set_xlabel("$x$")
    ax.set_ylabel("$y$")
    ax.set_xticks(np.linspace(-2.0, -2.0, 2))
    ax.set_yticks(np.linspace(-2.0, -2.0, 2))
...
if __name__ == "__main__":
    ...
    ### parser.add_argument("--log", default="../training/logs/linear_point_cbf/relu/commit_86a2dc2/version_3/checkpoints/epoch=101-step=17951.ckpt")
    ### parser.add_argument("--log", default="../training/logs/linear_point_cbf/relu/commit_86a2dc2/version_4/checkpoints/epoch=124-step=21999.ckpt")
    # parser.add_argument("--log", default="../training/logs/linear_point_cbf/relu/commit_86a2dc2/version_5/checkpoints/epoch=120-step=21295.ckpt")
    # parser.add_argument("--log", default="../training/logs/linear_point_cbf/tanh/commit_86a2dc2/version_0/checkpoints/epoch=113-step=20063.ckpt")
    parser.add_argument("--log", default="../training/logs/linear_point_cbf/relu/commit_86a2dc2/version_6/checkpoints/epoch=102-step=18127.ckpt")

    # eval_inverted_pendulum()
    args = parser.parse_args()
    
    plot_linear_satellite(args)
